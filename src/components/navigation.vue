	...
	<div>
		<div class="p-3 bg-neutral-900 fixed top-0 px-7 w-full z-50">
			<div class="flex justify-between items-center">
				<div>
					<h1 class="text-lg font-semibold">{{ $route.name === "zoeken" ? "Zoeken" : $route.query.title ? "Album" : "Albums" }}</h1>
					<p class="-mt-1 text-sm opacity-70">{{ $route.query.title ? "Alle nummers van " + $route.query.title : "Overzicht met alle albums" }}</p>
				</div>
...
<script setup>
	...
	const Zoeken = (event) => {
		const data = useSongData(false, false, invoer.value);
		console.log(data);
